...
APPROX_QUANTILES(altLENGTH,10) altTextLENGTHPercentiles,
 COUNT(srcset) srcsetCOUNT, COUNT(sizes) sizeCOUNT, COUNT(srcCOUNT) srcCOUNT, COUNT(altCOUNT) altCOUNT, count(flat_img) imgTagCount
...
SELECT 
     ...
     srcset, sizes, alt, flat_img, client
FROM(

SELECT REGEXP_EXTRACT(flat_img,r'srcset=".*?"') srcset, REGEXP_EXTRACT(flat_img,r'sizes=".*?"') sizes,
REGEXP_EXTRACT(flat_img,r'src=".*?"') src, REGEXP_EXTRACT(flat_img,r'alt=".*?"') alt, flat_img, client
...
FROM  `httparchive.response_bodies.2019_07_01_*` srcsets
